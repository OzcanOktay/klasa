...
module.exports = class extends Monitor {
	...
	async run(message) {
		...
		var command = this.client.commands.find(r=> r.name(this.client.languages.get(message.guild.settings.language)) == commandText);
		const aliasesbul = this.client.commands.filter(f=> f.aliases.includes(commandText))
		if (!command && aliasesbul.size == 0) return this.client.emit('commandUnknown', message, commandText, prefix, prefixLength);
		if (aliasesbul.size !== 0) command = this.client.commands.find(r=> r.name(this.client.languages.get(message.guild.language)) == aliasesbul.first().name(this.client.languages.get(message.guild.language)));
