...
class TextPrompt {
	...
	async reprompt(prompt) {
		...
		.setTitle(this.message.guild.language.get('komutgirisi'))
		.setDescription(this.message.guild.language.get('MONITOR_COMMAND_HANDLER_REPROMPT', prompt))
		.setColor('RANDOM')
		.setFooter(this.message.guild.language.get('MMONITOR_COMMAND_HANDLER_REPROMPT'))
		const message = await this.message.prompt(embed, this.time)
		// this.message.prompt(this.message.language.get('MONITOR_COMMAND_HANDLER_REPROMPT', `<@!${this.message.author.id}>`, prompt), this.time);

		this.responses.set(message.id, message);
var emb = new MessageEmbed()
.setColor("RANDOM")
.setDescription(this.message.language.get('MONITOR_COMMAND_HANDLER_ABORTED'))
.setTitle(this.message.language.get("KOMUT_TITLE"))
.setFooter(this.message.language.get("KOMUT_IPTAL_EDILDI", this.message), this.message.author.displayAvatarURL({format: "png"}))
		if (possibleAbortOptions.includes(message.content.toLowerCase())) throw emb
	...
	async repeatingPrompt() {
		...
		try {
			message = await this.message.prompt(
				this.message.language.get('MONITOR_COMMAND_HANDLER_REPEATING_REPROMPT', `<@!${this.message.author.id}>`, this._currentUsage.possibles[0].name, this.time / 1000, possibleCancelOptions), this.time);
			this.responses.set(message.id, message);
		} catch (err) {
			return this.validateArgs();
		}
