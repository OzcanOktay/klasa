...
class Command extends AliasPiece {
	...
	constructor(client, store, file, directory, options = {}) {
		...
		this.name = isFunction(options.name) ?
		(language = this.client.languages.default) => options.name(language) :
		options.name;

		if (options.autoAliases) {
			if (this.name().includes('-')) this.aliases.push(this.name().replace(/-/g, ''));
			this.aliases.push(this.name(this.client.languages.get('tr-TR')).replace(/-/g, ''))
			this.aliases.push(this.name(this.client.languages.get('tr-TR')))
			this.aliases.push(this.name(this.client.languages.get('en-US')).replace(/-/g, ''))
			this.aliases.push(this.name(this.client.languages.get('en-US')))
	...
	toJSON() {
		return {
			...
			name: isFunction(this.name) ? this.name() : this.name,
